...
def dispatch_value_type(ps, value_type, ref_ifg, ifg_list):
    ...
    if value_type == "v-dmos":
        ph = dispatch_value_type(ps, "u-dmos", ref_ifg, ifg_list)[0]
        ph_all = estimate_velocity_from_phase(ps, ph)
        return ph_all, "v-dmos", "mm/yr", ps_setref(ps)
    if value_type == "v-dms":
        ph = dispatch_value_type(ps, "u-dms", ref_ifg, ifg_list)[0]
        ph_all = estimate_velocity_from_phase(ps, ph)
        return ph_all, "v-dms", "mm/yr", ps_setref(ps)
